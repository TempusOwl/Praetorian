...
DEFAULTPREFIX = "!"
secret = utils.json_loader.read_json("secrets")
config = utils.json_loader.read_json("config")
bot = commands.Bot(
    command_prefix=get_prefix,
    case_insensitive=True,
    owner_id=134118092082118657,
    help_command=None,
    intents=intents,
)  # change command_prefix='-' to command_prefix=get_prefix for custom prefixes
bot.config_token = secret["token"]
bot.connection_url = secret["mongo"]

bot.news_api_key = secret["news api"]
bot.joke_api_key = secret["x-rapidapi-key"]
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Game(name="Watching Guard")
